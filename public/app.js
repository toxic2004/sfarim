...
function renderBooks() {
  ...
  grid.innerHTML = visible.map((book) => {
    ...
    const whatsappText = encodeURIComponent(`שלום, אני מתעניין בספר: ${book.title}, במחיר ${book.price} ₪`);
    return `<article class="book-card${sold ? " book-card--sold" : ""}">
      <div class="book-card__image-wrap">
        ...
        <span class="book-card__price"><bdi>${book.price} ₪</bdi></span>
...
fetch(STATUS_API, { cache: "no-store" })
  ...
  .then((data) => { statuses = data.statuses || {}; })
  .catch(() => { loading.textContent = "לא ניתן היה לרענן זמינות כרגע. אפשר עדיין לעיין בספרים ולשלוח הודעה."; })
